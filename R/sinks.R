...
#' sinks(integers())
#' sink_is_between(integers())
#' 
#' # The set 1, 1/2, 1/4, 1/8, ... has a sink at 0 approached from the right.
#' halves <- 0.5^natural0()
#' sinks(halves)
#' sink_is_between(halves, to = 0)
#' sink_is_between(halves, from = 0)
#' 
...
#' sinks(reciprocals)
#' test_discrete(reciprocals, Inf) # Yet Inf is a member.
#' @rdname sinks
#' @export
sinks <- function(x) {
  if (is.numeric(x)) {
