...
when we check whether a value is in the series at the underlying level.
...
- **Numeric-vector-based series** (plain numeric vectors or
  ...
  we decide if a value is in the series by checking whether it is within
...
This means that “is this value in the series?” fails for the eagerly
transformed vector, but not when the vector is preserved. This is
...
- **Increase `tol` for noisy values**: if your values come from
  computation or measurement and you are noticing values incorrectly
  treated as not in the series, increase `tol` accordingly.
