...
test_that("sinks - arithmetic and integers", {
  ...
  expect_true(sink_is_between(x))
  expect_false(sink_is_between(x, -10, 10))
  
  y <- integers()
  expect_sinks(y, c(-Inf, Inf), c(1, -1))
  expect_true(sink_is_between(y))
  expect_false(sink_is_between(y, -10, 10))
  
  z <- natural1()
  expect_sinks(z, Inf, -1)
  expect_true(sink_is_between(z))
  expect_false(sink_is_between(z, to = 10))
  
  w <- -natural1()
  expect_sinks(w, -Inf, 1)
  expect_true(sink_is_between(w))
  expect_false(sink_is_between(w, from = -10))
  
  # Numeric
  expect_sinks(1:10, numeric(), numeric())
  expect_sinks(dsct_numeric(1:10), numeric(), numeric())
  expect_false(sink_is_between(1:10))
  expect_false(sink_is_between(dsct_numeric(1:10)))
})
...
test_that("Inverse and union handles sinks properly.", {
  x <- 1 / integers()
  expect_true(sink_is_between(x, from = 0, to = 1))
  expect_true(sink_is_between(x, from = -1, to = 0))
  ...
  expect_true(sink_is_between(y, to = -1000))
  expect_true(sink_is_between(y, from = 1000))
  expect_true(sink_is_between(y, from = -1, to = -0.1))
  expect_false(sink_is_between(y, from = -0.1, to = 0))
  expect_true(sink_is_between(y, from = 0, to = 0.1))
  
  z_base <- 1 / natural1()
  z <- dsct_union(z_base, -z_base, z_base + 1)
  expect_sinks(z, c(0, 0, 1), c(-1, 1, 1))
  expect_true(sink_is_between(z))
  expect_false(sink_is_between(z, 0.5, 1))
  expect_true(sink_is_between(z, to = 0))
  expect_true(sink_is_between(z, from = 1))
...
test_that("Transformations preserve sinks.", {
  ...
  expect_true(sink_is_between(y, from = 1, to = 2))
  expect_true(sink_is_between(y, from = 0.5, to = 1))
  expect_false(sink_is_between(y, from = 2))
  expect_false(sink_is_between(y, to = 0.5))
  expect_false(sink_is_between(y, to = 0))
  expect_false(sink_is_between(y, to = -1))
  ...
  expect_sinks(x, 0, 1)
  expect_true(sink_is_between(x))
  expect_false(sink_is_between(x, to = 0))
