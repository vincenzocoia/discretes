test_that("Ops with length-0 vectors", {
  ...
  expect_identical(integers() + integer(), empty_set("integer"))
  expect_identical(integers() * numeric(), empty_set("numeric"))
  expect_identical(integers() + numeric(), empty_set("numeric"))
  expect_identical(integers() ^ integer(), empty_set("numeric"))
  expect_identical(integer() ^ integers(), empty_set("numeric"))
  expect_identical(integers() ^ numeric(), empty_set("numeric"))
  expect_identical(numeric() ^ integers(), empty_set("numeric"))
  # Length-0 series
...
test_that("^ works for number^discretes", {
  ...
  zeroone <- 0^integers()
  expect_equal(get_discretes_in(zeroone), c(0, 1))
  ...
  infone <- Inf^integers()
  expect_equal(get_discretes_in(infone), c(1, Inf))
